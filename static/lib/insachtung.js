...
(function() {
  var ACTION, Bitmap, Bonus, Controller, CrashController, NoWall, Painter, Player, PlayerInstance, Position, Round, canvas, controller, playerSelectionForm, playersConfiguration,
  ...
  Painter = (function() {
    ...
    Painter.prototype.paintPositions = function(player) {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        position = _ref[_i];
        if (position.action === ACTION.MOVE_TO) {
          this.context.moveTo(position.x, position.y);
        } else if (position.action === ACTION.LINE_TO) {
    ...
    Painter.prototype.paintHead = function(player) {
      return this.drawCircle(player.lastPos().x, player.lastPos().y, player.size / 2, "yellow");
    };
  ...
  NoWall = (function(_super) {
    ...
    NoWall.prototype.play = function() {
      --this.duration;
      return this.player.lastPos().action = ACTION.MOVE_TO;
    };
    ...
    Round.prototype.launch = function() {
      ...
      main = function() {
        ...
        if (!_this.isOver) {
          ...
          _this.controller.painter.clearBoard();
  ...
  PlayerInstance = (function() {
    ...
    function PlayerInstance(_static) {
      ...
      this.size = 5;
      this.bonuses = [];
      this.positions = [new Position(Math.floor((Math.random() * 700) + 100), Math.floor((Math.random() * 700) + 100), ACTION.MOVE_TO)];
    ...
    PlayerInstance.prototype.updatePos = function() {
      return this.positions.push(new Position(this.lastPos().x + Math.cos(this.course) * this.radius, this.lastPos().y + Math.sin(this.course) * this.radius, ACTION.LINE_TO));
    };
    ...
    PlayerInstance.prototype.paint = function() {
      this["static"].painter.paintPositions(this);
  ...
  CrashController = (function() {
    ...
    CrashController.prototype.checkForCrashes = function(player) {
      if (player.lastPos().x < this.controller.bitmap.boundariesWidth || player.lastPos().x > this.controller.bitmap.width - this.controller.bitmap.boundariesWidth || player.lastPos().y < this.controller.bitmap.boundariesWidth || player.lastPos().y > this.controller.bitmap.height - this.controller.bitmap.boundariesWidth) {
